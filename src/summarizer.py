...
4. mvp：素人可複製的最小版本——一個不會寫程式的普通人，想試這個模式，
   「今天就能動手」的最小起步是什麼（1~2 句）。要具體到第一步做什麼、做給誰、
   用什麼現成工具（如 ChatGPT/Claude/現成平台），不要寫「先學會寫程式」這種大門檻。
5. evidence：收錢證據，照實分級寫（每項輸入資料裡的 site_check 欄位是程式
   實際訪問產品官網的查證結果，可直接引用）：
   - 貼文裡有具體數字 → 寫「作者自稱 MRR $8k」——自稱就寫自稱，不要寫成事實。
   - site_check 非空 → 加上「官網有定價（已查證）」。
   - 兩者都有就都寫；都沒有就給空字串 ""。
6. rating：1~5 的整數，代表「對想用 AI 一人創業的讀者」的幫助程度。
   請誠實給分，不要每則都給高分。
   【硬規則，依證據等級封頂】
   - 貼文沒有具體收錢數字、site_check 也是空的 → 最高 2（故事再精彩也一樣）。
     免費產品、純心得文、還沒開始收費的專案都屬於這類。
   - 只有官網定價、貼文沒有數字 → 最高 3（有收費模式，但不知道有沒有人買）。
   - 有具體營收/付費用戶數字＋可複製做法 → 才可能 4~5。
7. 最多輸出 {max_items} 則，依 rating 由高到低排序。
8. 嚴格只回傳 JSON，不要有 markdown 圍欄或多餘文字。

輸出格式：
{{
  "headline": "今日案例一句話總覽",
  "items": [
    {{"title": "精簡標題（含營收數字更好）", "rating": 4, "insight": "案例做什麼、成果、怎麼做到", "takeaway": "對讀者的一句話啟發", "barrier": "門檻與天花板", "mvp": "素人今天能動手的最小版本", "evidence": "作者自稱 MRR $8k；官網有定價（已查證）", "source": "來源", "url": "原文連結"}}
  ]
}}
"""
